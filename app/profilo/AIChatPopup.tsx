...
export default function AIChatPopup() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      ...
      {/* FINESTRA POPUP (Visibile solo se isOpen è true) */}
      {isOpen && (
        <div className="w-80 md:w-96 h-[500px] bg-[#1c1f27] border border-slate-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in slide-in-from-bottom-10 fade-in duration-200">
            ...
            <div className="p-4 bg-black/40 border-b border-slate-800 flex justify-between items-center">
                <h3 className="font-bold text-white text-sm">Nexum Assistant</h3>
      ...
      {/* BOTTONE TONDO (Trigger) */}
      <button 
        onClick={() => setIsOpen(!isOpen)}
        className="w-14 h-14 rounded-full bg-[#7c3aed] hover:bg-[#6d28d9] text-white shadow-lg shadow-[#7c3aed]/20 flex items-center justify-center transition-all hover:scale-105 active:scale-95"
      >
        {isOpen ? (
